...
result = {"currencies": {}}
orig_countries = process_countries('en')

for country in orig_countries["countries"]:
    for unit in country["units"]:
        result["currencies"][unit] = unit
    if "contemporary_units" in country:
        for unit in country["contemporary_units"]:
            result["currencies"][unit] = unit
        
with open(f"../i18n/currencies_en.json", 'w', encoding='utf8') as json_file:
...
#for lang in ('bg',):
    ...
    for orig_country in orig_countries["countries"]:
        ...
        for i, unit in enumerate(orig_units):
